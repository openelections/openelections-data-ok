#The following code is based on a revised copy of the 2004 oklahoma State House results.
#Formatting issues that arose when tranforming it to a Tabula document, required me to manually indent columns/rows.
#Several cells combined multiple cells of data that had to be split apart, and spaces removed.

#Both a revised and original copy of the 2004 Oklahoma State House results are avaialble through the openelections github repo.
...
def parser_2004():
  election_results = []
  original_df = pd.read_csv('../tabula/2004/tabula-04sh_revised.csv',header=None)
...
def county_split(df_index, df, return_list,election_results):
  ...
  while df.ix[county_index][0] != 'STATE TOTAL:' and df.ix[county_index][0] != 'STATE TOTAL: ':
...
def election_totals(results_string):
  ...
  # Due to constraints with data formatting, I was required to alter the names of candidates in the data tables.  
  # Following code snippet, takes the altered data and replaces it with the original names from the file.
  candidate_df.loc[candidate_df['Candidate'].str.contains('JOHN YOUNG'),'Candidate'] = 'John Mark Young'
  candidate_df.loc[candidate_df['Candidate'].str.contains('RICHARD PRAWDZIENSKI'),'Candidate'] = 'Richard Ski Prawdzienski'
  candidate_df.loc[candidate_df['Candidate'].str.contains('RAYMOND McCARTER'),'Candidate'] = 'Raymond Gene McCarter'
  candidate_df.loc[candidate_df['Candidate'].str.contains('ROY HOOPER'),'Candidate'] = 'Roy B. Butch Hooper'
  candidate_df.loc[candidate_df['Candidate'].str.contains('RUSSELL TURNER'),'Candidate'] = 'Russell Don Turner'
  candidate_df.loc[candidate_df['Candidate'].str.contains('RICHARD MORRISSETTE'),'Candidate'] = 'Richard Daniel Morrissette'
  candidate_df.to_csv('../2004/20041102__ok__general__state_house__county.csv',index=False)
