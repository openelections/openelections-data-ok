...
def parse_2012():
    ...
    '''
    df = pd.read_csv(
        'http://www.ok.gov/elections/support/12gen_prec_csv.zip')
    ...
    assert dft.loc['President'].total_votes.sum() == 1334872
    assert dft.loc['U.S. House'].total_votes.sum() == 1325935

    df.to_csv('2012/20121106__ok__general__precinct.csv', index=False)
